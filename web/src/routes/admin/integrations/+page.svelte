...
<Dialog bind:open={modalOpen} title={editing ? `Configure ${editing.name}` : `Add ${selected?.name ?? 'integration'}`} description="Credentials are encrypted and never returned after saving." size="xl">
	{#if selected}<form onsubmit={(e) => { e.preventDefault(); save(); }} class="space-y-4"><label class="block"><span class="text-sm text-rig-300">Instance name</span><input class={fieldClass} bind:value={formName} required /></label>{#each selected.config as f (f.key)}<label class="block"><span class="text-sm text-rig-300">{f.label}{f.required ? ' *' : ''}</span>{#if f.type === 'select'}<Select class="mt-1" bind:value={formConfig[f.key]} items={(f.options ?? []).map((option) => ({ value: option, label: option }))} />{:else}<input class={fieldClass} type={f.type === 'password' ? 'password' : f.type} bind:value={formConfig[f.key]} placeholder={editing && f.secret && editing.secretFields?.includes(f.key) ? 'Saved — leave blank to keep' : f.placeholder} required={f.required && !(editing && editing.secretFields?.includes(f.key))} />{/if}{#if f.help}<span class="mt-1 block text-xs text-rig-500">{f.help}</span>{/if}</label>{/each}<div class="flex justify-end gap-2 border-t border-rig-800 pt-4"><button type="button" onclick={() => modalOpen = false} class="rounded-md px-4 py-2 text-sm text-rig-300 hover:bg-rig-800">Cancel</button><button type="submit" disabled={saving} class="rounded-md bg-rig-500 px-4 py-2 text-sm font-medium text-rig-950 disabled:opacity-50">{saving ? 'Saving…' : editing ? 'Save changes' : 'Create instance'}</button></div></form>{/if}
</Dialog>
